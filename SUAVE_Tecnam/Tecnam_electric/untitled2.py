...
def vehicle_setup():
    ...
    vehicle.mass_properties.operating_empty           = 1230 * Units.kilogram  
    vehicle.mass_properties.max_zero_fuel             = 1161 * Units.kilogram
    ...
    fuel.mass_properties.mass = 0.0 * Units.kilogram #IMPORTANT
    fuel.mass_properties.center_of_gravity = np.array([[3.711*Units.meter,0.0,1.077* Units.meter]]) #IMPORTANT
    fuel.number_of_tanks = 0 #IMPORTANT
    fuel.internal_volume = 0.001 * Units['meters**3'] #IMPORTANT
    fuel.density = 0.1 * (Units.kilogram/Units['meters**3']) #IMPORTANT
    ...
    batteryprop = SUAVE.Components.Energy.Networks.Battery_Propeller_Lo_Fid()
    batteryprop.tag = 'propulsor'
    
    # setup
    batteryprop.number_of_engines = 2
    batteryprop.engine_length     = 1.2 * Units.meter  #1.74
    batteryprop.thrust_angle      = 0.0 * Units.degrees
    batteryprop.rated_speed       = 2700.0 * Units['rpm']  #5800.0
    batteryprop.rated_power       = 72000.0 * Units.watts #For Weight Analysis  #149139.9744
    batteryprop.nacelle_diameter  = 0.1732        #0.58
    batteryprop.voltage           = 72000.0 * Units.watts
    
    #compute engine areas
    batteryprop.areas.wetted      = 1.1*np.pi*batteryprop.nacelle_diameter*batteryprop.engine_length
    ...
    prop_attributes.tip_radius       = 0.89 * Units.meter
    prop_attributes.propulsive_efficiency = 0.95
    prop_attributes.origin              = [[2.494* Units.meter,1.6* Units.meter,1.077* Units.meter]]
    prop_attributes.number_blades       = 2
    prop_attributes.freestream_velocity = 77.17 * Units['m/s'] # freestream m/s
    prop_attributes.angular_velocity    = 607.38 * Units['rad/s'] # For 20x10 prop
    ...
    batteryprop.propeller = prop_attributes 
    ...
    #engine.speed              = 5800.0 * Units['rpm']
    #engine.power_specific_fuel_consumption     =  0.83454 #4.19
    ...
    motor.speed_constant       = 180.0 * Units['rpm/volt'] # RPM/volt is standard
    motor                      = size_from_kv(motor)
    motor.propeller_radius     = batteryprop.propeller.tip_radius
    motor.gear_ratio           = 1. # Gear ratio, no gearbox
    motor.gearbox_efficiency   = .98 # Gear box efficiency, no gearbox
    motor.expected_current     = batteryprop.voltage/motor.resistance
    
    batteryprop.motor              = motor
    ...
    # ------------------------------------------------------------------
    # Component 4 - Avionics
    
    avionics = SUAVE.Components.Energy.Peripherals.Avionics()
    avionics.power_draw = 50.0 * Units.watts
    ...
    payload.power_draw           = 51.3 * Units.watts 
    ...
    bat.mass_properties.mass = 151.0 * Units.kg
    bat.specific_energy      = 26500.0 * Units.Wh/Units.kg  #265.0  #192.84
    bat.specific_power       = 0.34 * Units.kW/Units.kg  #0.837
    bat.resistance           = 0.0153 * Units['ohm']
    bat.max_voltage          = 72000.0 * Units.watts
    initialize_from_mass(bat,bat.mass_properties.mass)
    batteryprop.battery              = bat
    
    
    # Recalculating mass properties based on battery
...
def mission_setup(analyses, vehicle):
    ...
    segment.battery_energy = vehicle.base.propulsors.propulsor.battery.max_energy * 0.95
    segment.state.unknowns.throttle = 0.8 * ones_row(1)
    segment.altitude_start = 0.0   * Units.meter
    segment.altitude_end   = 2438.4   * Units.meter
    segment.climb_rate     = 1.0 * Units['m/s']
    segment.mach_start     = 0.05    #0.05
    segment.mach_end       = 0.08    #0.1
    ...
    segment.mach  = 0.077        #0.087       #0.099
    segment.distance   = 7.0 * Units.nautical_miles
