...
def vehicle_setup():
    ...
    network.tag = 'propulsor'
    
    # setup
    network.number_of_engines = 2
    network.engine_length     = 1.2 * Units.meter
    network.thrust_angle      = 0.0 * Units.degrees
    network.rated_speed       = 2700 * Units['rpm']
    network.rated_power       = 72000.0 * Units.watts #For Weight Analysis
    network.nacelle_diameter = 0.1732
    network.voltage          = 72000.0 * Units.watts
    
    #compute engine areas
    netowrk.areas.wetted      = 1.1*np.pi*network.nacelle_diameter*network.engine_length
    ...
    prop_attributes.freestream_velocity = 140 * Units.knots # freestream m/s
    prop_attributes.angular_velocity    = 2390 * Units['rpm'] # For 20x10 prop
    ...
    network.propeller = prop_attributes 
    ...
    motor.propeller_radius     = network.propeller.tip_radius
    motor.gear_ratio           = 1. # Gear ratio, no gearbox
    motor.gearbox_efficiency   = .98 # Gear box efficiency, no gearbox
    motor.expected_current     = network.voltage/motor.resistance
    network.motor              = motor
    ...
    bat.mass_properties.mass = 181.44 * Units.kg
    bat.specific_energy      = 265.0 * Units.Wh/Units.kg  #192.84
...
def mission_setup(analyses):
    ...
    segment.air_speed      = 58 * Units.knots
    ...
    segment.air_speed    = 58 * Units.knots
    segment.descent_rate = 4.0 * Units['m/s']
