...
def main():   
    ...
    x_data = pd.read_csv("x_6.csv")
    x_data = x_data[['alt', 'alpha', 'mach', 'rpm', 'cp', 'ct']]
    
    ## Drag Data
    
    # Skin Friction Drag Coefficient (Estimated parasite drag) Breakdown
    
    cdo_fuselage_data = pd.read_csv("Fuselage_CDo_6.csv")
    cdo_fuselage_data = cdo_fuselage_data[['Fuselage_CDo']]
    cdo_hs_data = pd.read_csv("Horizontal_Stab_CDo_6.csv")
    cdo_hs_data = cdo_hs_data[['Horizontal_Stab_CDo']]
    cdo_vs_data = pd.read_csv("Vertical_Stab_CDo_6.csv")
    cdo_vs_data = cdo_vs_data[['Vertical_Stab_CDo']]
    cdo_wing_data = pd.read_csv("Main_Wing_CDo_6.csv")
    cdo_wing_data = cdo_wing_data[['Main_Wing_CDo']]
    
    # Integrated Induced Drag Coefficient Breakdown
    
    cdi_fuselage_data = pd.read_csv("Fuselage_CDi_6.csv")
    cdi_fuselage_data = cdi_fuselage_data[['Fuselage_CDi']]
    cdi_hs_data = pd.read_csv("Horizontal_Stab_CDi_6.csv")
    cdi_hs_data = cdi_hs_data[['Horizontal_Stab_CDi']]
    cdi_vs_data = pd.read_csv("Vertical_Stab_CDi_6.csv")
    cdi_vs_data = cdi_vs_data[['Vertical_Stab_CDi']]
    cdi_wing_data = pd.read_csv("Main_Wing_CDi_6.csv")
    cdi_wing_data = cdi_wing_data[['Main_Wing_CDi']]
    
    # Total Aircraft Drag
    
    cdo_total_data = pd.read_csv("CDo_6.csv")
    cdo_total_data = cdo_total_data[['CDo']]
    cdi_total_data = pd.read_csv("CDi_6.csv")
    cdi_total_data = cdi_total_data[['CDi']]
    cd_total_data = pd.read_csv("CDtotal_6.csv")
    ...
    cl_fuselage_data = pd.read_csv("Fuselage_CL_6.csv")
    cl_fuselage_data = cl_fuselage_data[['Fuselage_CL']]
    cl_hs_data = pd.read_csv("Horizontal_Stab_CL_6.csv")
    cl_hs_data = cl_hs_data[['Horizontal_Stab_CL']]
    cl_vs_data = pd.read_csv("Vertical_Stab_CL_6.csv")
    cl_vs_data = cl_vs_data[['Vertical_Stab_CL']]
    cl_wing_data = pd.read_csv("Main_Wing_CL_6.csv")
    cl_wing_data = cl_wing_data[['Main_Wing_CL']]
    
    # Total Aircraft Lift
    
    cl_total_data = pd.read_csv("CL_6.csv")
    ...
    x_data.values[:,3]=x_data.values[:,3]/rpm_range[1]
    ...
    outliers = pd.DataFrame((x_data['CL'] > 4) | (x_data['CL'] < -2) | (x_data['CDtotal'] > 1.5) | (x_data['CDtotal'] < -1))
    outliers.describe()
    ...
    x_data_out = x_data.mask((x_data['CL'] > 4) | (x_data['CL'] < -2) | (x_data['CDtotal'] > 1.5) | (x_data['CDtotal'] < -1)).dropna()   
    ...
    #regr_cdi_hs=MLPRegressor(activation= 'relu', solver='lbfgs', tol=1e-10, alpha=1e-10, max_iter=30500, random_state=1)
    #regr_cdi_hs = MLPRegressor(activation= 'relu', solver='adam', tol=1e-55, alpha=1e-25, max_iter=30500, random_state=1, beta_1 = 0.99, beta_2 = 0.99, epsilon=1e-50)
    ...
    #regr_cdi_wing = MLPRegressor(activation= 'relu', solver='adam', tol=1e-55, alpha=1e-25, max_iter=30500, random_state=7, beta_1 = 0.99, beta_2 = 0.99, epsilon=1e-50)
    ...
    #regr_cl_hs = MLPRegressor(activation= 'relu', solver='lbfgs', tol=1e-10, alpha=1e-10, max_iter=30500, random_state=1)
    #regr_cl_vs = MLPRegressor(activation= 'relu', solver='adam', tol=1e-100, alpha=1e-50, max_iter=305000, random_state=1, beta_1 = 0.999, beta_2 = 0.999, epsilon=1e-10)
    ...
    #                              alpha=1e-55, max_iter=30500, random_state=8, beta_1 = 0.4, beta_2 = 0.99, epsilon=1e-50, hidden_layer_sizes=(1000,1000))
    #regr_cd_total = MLPRegressor(activation= 'relu', solver='lbfgs', tol=1e-10, alpha=1e-10, max_iter=30500, random_state=1)
    #regr_cl_total = MLPRegressor(activation= 'relu', solver='adam', learning_rate='adaptive', learning_rate_init=0.0001, tol=1e-55, 
                                 ...
                                 #n_iter_no_change= 700,hidden_layer_sizes=(1000,1000))
    #regr_cl_total = MLPRegressor(activation= 'relu', solver='lbfgs', tol=1e-100, alpha=1e-10, max_iter=8500, random_state=1)
    ...
    #filename = 'NN_Surrogate_Model_MLPRegressor_CDi_HS_3.sav'
    #pickle.dump(regr_cdi_hs, open(filename, 'wb'))
    
    #filename = 'NN_Surrogate_Model_MLPRegressor_CDi_VS_2.sav'
    #pickle.dump(regr_cdi_vs, open(filename, 'wb'))
    
    filename = 'NN_Surrogate_Model_MLPRegressor_CDi_Wing_3.sav'
    pickle.dump(regr_cdi_wing, open(filename, 'wb'))
    
    #filename = 'NN_Surrogate_Model_MLPRegressor_CL_HS_3.sav'
    #pickle.dump(regr_cl_hs, open(filename, 'wb'))
    
    #filename = 'NN_Surrogate_Model_MLPRegressor_CL_VS_3.sav'
    #pickle.dump(regr_cl_vs, open(filename, 'wb'))
    
    #filename = 'NN_Surrogate_Model_MLPRegressor_CL_Wing_3.sav'
    #pickle.dump(regr_cl_wing, open(filename, 'wb'))
    
    #filename = 'NN_Surrogate_Model_MLPRegressor_CDi_4.sav'
    #pickle.dump(regr_cdi_total, open(filename, 'wb'))
    
    #filename = 'NN_Surrogate_Model_MLPRegressor_CD_2.sav'
    #pickle.dump(regr_cd_total, open(filename, 'wb'))
    
    #filename = 'NN_Surrogate_Model_MLPRegressor_CL_3.sav'
